...
export const taxRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/years', async (_req) => {
    const rows = app.db
      .prepare('SELECT * FROM tax_year_config ORDER BY tax_year')
      .all() as unknown as TaxYearConfigRow[]
  ...
  app.get<{ Querystring: { taxYear?: string; income?: string } }>('/dividends', async (req) => {
    const user = req.session.user!
    const taxYear = req.query.taxYear ?? taxYearForDate(new Date().toISOString().slice(0, 10))
    ...
    const items = dividendTxns.map((txn) =>
      computeDividendTax(txn, config, taxYear, incomeAboveBasicRate),
    )
  ...
  app.get<{ Querystring: { taxYear?: string } }>('/espp', async (req) => {
    const user = req.session.user!
    const taxYear = req.query.taxYear ?? taxYearForDate(new Date().toISOString().slice(0, 10))
    ...
    const projectedIncomeGbp = projectedItems.reduce(
      (s, r) => s.plus(r.incomeAmountGbp ?? '0'),
      new Big(0),
    )

    const summary =
      items.length === 0 && projectedItems.length === 0
        ? null
        : {
  ...
  app.get<{ Querystring: { instrumentId: string } }>(
    ...
    async (req) => {
      ...
      const {
        ...
        applyStockSplit,
        applyCapReturn,
      } = await import('../services/tax/pool.ts')
      const { default: Big } = await import('big.js')
      let pool = emptyPool()
      const history: {
        txnId: number
