...
function periodBounds(period: string, today: string): [string, string] {
  ...
  switch (period) {
    ...
    case '3m':
      return [subtractDays(today, 90), today]
    case '6m':
      return [subtractDays(today, 182), today]
    case '1y':
      return [subtractDays(today, 365), today]
    case '2y':
      return [subtractDays(today, 730), today]
    case '3y':
      return [subtractDays(today, 1095), today]
    case '5y':
      return [subtractDays(today, 1825), today]
    case '7y':
      return [subtractDays(today, 2555), today]
    case '10y':
      return [subtractDays(today, 3650), today]
    case '15y':
      return [subtractDays(today, 5475), today]
    default:
      return [subtractDays(today, 365), today] // fallback: 1y
  }
}
...
export const chartRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get<{ Querystring: { period?: string } }>('/portfolio-value', async (req) => {
    ...
    for (const inst of instruments) {
      ...
      const earliest = app.db
        .prepare(`SELECT MIN(price_date) as d FROM price WHERE instrument_id = ?`)
  ...
  app.get<{ Querystring: { taxYear?: string } }>('/realised-gains', async (req) => {
    const user = req.session.user!
    const today = new Date().toISOString().slice(0, 10)
    ...
    if (taxYear) {
      const configRow = app.db
        .prepare('SELECT start_date, end_date FROM tax_year_config WHERE tax_year = ?')
  ...
  app.get<{ Querystring: { taxYear?: string } }>('/dividend-income', async (req) => {
    const user = req.session.user!
    const today = new Date().toISOString().slice(0, 10)
    const taxYear = req.query.taxYear
