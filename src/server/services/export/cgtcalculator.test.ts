...
import { formatCgtCalculator, toCgtCalculatorRows } from './cgtcalculator.ts'
...
function mkTxn(
  ...
  return {
    ...
    dividendGrossGbp: null,
    dividendWithholdingGbp: null,
    dividendNetGbp: null,
    splitRatio: null,
...
describe('cgtcalculator.com exporter', () => {
  ...
  it('maps RSU_VEST to B row', () => {
    const txns = [
      mkTxn({ txnType: 'RSU_VEST', txnDate: '2024-06-01', quantity: '25', unitPriceGbp: '19.50' }),
    ]
    const rows = toCgtCalculatorRows(txns, instrumentsById)
  ...
  it('skips non-price types like SPLIT', () => {
    const txns = [
      mkTxn({ txnType: 'SPLIT', txnDate: '2024-01-01', quantity: '200', unitPriceGbp: '0' }),
    ]
    expect(toCgtCalculatorRows(txns, instrumentsById)).toHaveLength(0)
  })
