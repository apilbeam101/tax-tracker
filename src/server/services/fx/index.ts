...
export interface FxService {
  ...
  convert(amount: string, fromCurrency: string, toCurrency: string, date: string): Promise<{ gbp: string; rate: FxRate }>
}
...
export function createFxService(store: FxRateStore, policy: FxRateType): FxService {
  async function getRate(fromCurrency: string, toCurrency: string, date: string): Promise<FxRate> {
    if (fromCurrency === toCurrency) {
      return {
        id: 0, fromCurrency, toCurrency, rateDate: date,
        rate: '1', rateType: policy, source: 'identity',
    ...
    if (fromCurrency === 'GBX' && toCurrency === 'GBP') {
      return {
        id: 0, fromCurrency, toCurrency, rateDate: date,
        rate: '0.01', rateType: policy, source: 'identity',
        fetchedAt: new Date().toISOString(),
      }
    }
    if (fromCurrency === 'GBP' && toCurrency === 'GBX') {
      return {
        id: 0, fromCurrency, toCurrency, rateDate: date,
        rate: '100', rateType: policy, source: 'identity',
        fetchedAt: new Date().toISOString(),
      }
    }
